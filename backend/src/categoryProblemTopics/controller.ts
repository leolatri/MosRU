...
export class CategoryProblemTopicController {
  ...
  async getCategoryProblemTopic(
    ...
    if (!relation)
      throw new NotFoundException(
        `Relation between object category ${objectCategoryId} and problem topic ${problemTopicId} not found`,
      );
  ...
  @Post()
  async createCategoryProblemTopic(
    @Body() dto: CategoryProblemTopicDTO,
  ): Promise<CategoryProblemTopicModel> {
    try {
      const result = await this.dbService.query<CategoryProblemTopicModel>(
      ...
      if (error instanceof DatabaseError && error.code === '23505') {
        throw new ConflictException(
          `This category and problem topic are already connected`,
        );
      }
