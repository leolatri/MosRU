...
export class ImportsController {
    ...
    async uploadXlsx(@UploadedFile() file: Express.Multer.File | undefined): Promise<ImportResult> {
        ...
        if (!hasXlsxExtension) throw new BadRequestException('Only .xlsx files are allowed');

        const hasValidMimeType =
            file.mimetype === XLSX_MIME_TYPE ||
            file.mimetype === 'application/octet-stream';

        if (!hasValidMimeType) throw new BadRequestException('Uploaded file has an invalid MIME type');
