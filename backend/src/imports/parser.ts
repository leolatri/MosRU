...
import {
  ImportRowError,
  NormalizedViolationRow,
  ParsedWorkbook,
  RawViolationRow,
  ValidationResult,
} from './models';
import { ADMINISTRATIVE_OKRUG_ALIASES } from './admOkrugs';
...
export class XlsxParserService {
  ...
  ): Promise<NormalizedViolationRow[]> {
  ...
  private validateHeaders(worksheet: ExcelJS.Worksheet): void {
    ...
    for (
      let columnNumber = EXPECTED_HEADERS.length + 1;
  ...
  private normalizeAdministrativeOkrugCode(
    value: string | null,
  ): string | null {
    if (value === null) {
      return null;
    ...
    const lookupKey = value.toLocaleLowerCase('ru-RU');

    return ADMINISTRATIVE_OKRUG_ALIASES[lookupKey] ?? value;
  }

  private validateRows(rawRows: RawViolationRow[]): ValidationResult {
    const districtOkrugRows = new Map<
      string,
      { okrugKey: string; okrugName: string; row: number }
    >();
    ...
    for (const rawRow of rawRows) {
      ...
      const administrativeOkrugText = this.normalizeNullableText(
        rawRow.administrativeOkrug,
      );

      const administrativeOkrug = this.normalizeAdministrativeOkrugCode(
        administrativeOkrugText,
      );
      ...
      const responseDeadlineText = this.normalizeNullableText(
        rawRow.responseDeadline,
      );
      ...
      if (!/^[1-9]\d*$/.test(applicationNumber)) {
        ...
        if (firstRow !== undefined) {
          ...
          applicationNumberRows.set(applicationNumber, rawRow.row);
        }
      }
      ...
      if (publicationDate === null) {
        rowErrors.push({
          ...
          message:
            'Дата публикации должна иметь формат ДД.ММ.ГГГГ и быть существующей датой',
        });
      }
      ...
      if (administrativeOkrug !== null && district !== null) {
        const districtKey = district.toLocaleLowerCase('ru-RU');
        const okrugKey = administrativeOkrug.toLocaleLowerCase('ru-RU');

        const previousRelation = districtOkrugRows.get(districtKey);

        if (
          previousRelation !== undefined &&
          previousRelation.okrugKey !== okrugKey
        ) {
          rowErrors.push({
            row: rawRow.row,
            field: 'district',
            message: `Район "${district}" уже связан с округом "${previousRelation.okrugName}" в строке ${previousRelation.row}`,
          });
        } else if (previousRelation === undefined) {
          districtOkrugRows.set(districtKey, {
            okrugKey,
            okrugName: administrativeOkrug,
            row: rawRow.row,
          });
        }
      }
      ...
      const responseDeadline =
        responseDeadlineText === null
          ? null
          : this.normalizeDate(responseDeadlineText);
      ...
      if (
        ...
        responseDeadline < publicationDate
      ) {
        rowErrors.push({
          row: rawRow.row,
          field: 'responseDeadline',
