...
export class ObjCategoriesController {
    ...
    @Get(':id')
    async getObjectCategoriesById(@Param('id', ParseIntPipe) id: number): Promise<ObjCategoriesModel> {
        const result = await this.dbService.query<ObjCategoriesModel>(
            `SELECT id, name
            FROM object_categories
            WHERE id = $1
    ...
    @Delete(':id')
    async deleteObjectCategories(@Param('id', ParseIntPipe) id: number): Promise<ObjCategoriesModel> {
        const result = await this.dbService.query<ObjCategoriesModel>(
