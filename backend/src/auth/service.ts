import { ConflictException, Injectable, NotFoundException, OnModuleDestroy, UnauthorizedException } from "@nestjs/common";
...
export class AuthService {
    ...
    async registration(dto: UserDTO): Promise<UserModel> {
        ...
        try {
            ...
            if (error instanceof DatabaseError && error.code === '23505') throw new ConflictException(`User with this email already exists`);
            throw error;
        }
    };
    ...
    async login(dto: UserDTO) {
        ...
        if (!passwordEqual) throw new UnauthorizedException(`Invalid email or password`);
