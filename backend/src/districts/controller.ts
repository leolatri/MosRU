...
export class DistrictsController {
    ...
    async getDistrictById(@Param('id', ParseIntPipe) id: number): Promise<DistrictModel> {
        ...
        if (!district) throw new NotFoundException(`Not found district with id = ${id}`)
    ...
    async createDistrict(@Body() dto: DistrictsDTO): Promise<DistrictModel> {
        try {
            ...
            if (error instanceof DatabaseError && error.code === '23505') throw new ConflictException(`District with name = ${dto.name} is alrady exist`);
            throw error;
        }
    };
    ...
    @Delete(':id')
    async deleteDistrict(@Param('id', ParseIntPipe) id: number): Promise<DistrictModel> {
        const result = await this.dbService.query<DistrictModel>(
